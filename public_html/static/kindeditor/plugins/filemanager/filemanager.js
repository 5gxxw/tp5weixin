...
KindEditor.plugin('filemanager', function(K) {
	...
	self.plugin.filemanagerDialog = function(options) {
		...
		function bindEvent(el, result, data, createFunc) {
			...
			var current_url = result.current_url.replace('http://ning.img.wxbuluo2.com\\','');
			var fileUrl = K.formatUrl(current_url + data.filename, 'absolute'),
		...
		function createView(result) {
			...
			$(".xl_span").click(function(){//这里的.xl_span对应着刚才插入的删除按钮上的class
			    ...
			    $.post('/static/kindeditor/php/file_manager_json.php',{action:"delete",url:$this.attr("data-url")},function(res){//jquery的post，action为指定的配合后端用，url是获取刚才存在删除按钮上的图片路径，你完全可以用别的来写，因为用不好kindeditor的js库才用的jquery
